...
function ActivityCategoryPicker({
  ...
  const { isLoading, data, error } = useGraphqlQuery(
    `query CommCategoryPicker($search: String, $first: Int, $isActive: Boolean) {
      activityCategory(name_Icontains: $search, first: $first, isActive: $isActive) {
        edges { node { id code name } }
      }
    }`, filters,
  );
  const options = data?.activityCategory?.edges?.map((e) => e.node) ?? [];
  return (
    <Autocomplete
      error={error}
      readOnly={readOnly}
      ...
      isLoading={isLoading}
      value={value}
      label={label || formatMessage('communications.categoryPicker')}
      withLabel={withLabel}
      required={required}
      ...
      renderInput={(inputProps) => (
        ...
        <TextField {...inputProps} required={required} label={label || formatMessage('communications.categoryPicker')} />
      )}
    />
  );
}
